...
resource "azurerm_linux_web_app" "alwa" {
  name                = "task-board-webapp-${random_integer.ri.result}"
  resource_group_name = azurerm_resource_group.rg.name
...
resource "azurerm_app_service_source_control" "github" {
  ...
  repo_url               = "https://github.com/MartotoBG/azure-web-app"
  branch                 = "main"
...
resource "azurerm_mssql_server" "mssql" {
  ...
  location                     = azurerm_resource_group.rg.location
  version                      = "12.0"
  administrator_login          = "martotobg123"
  administrator_login_password = "obichamKotki123!"
}
...
resource "azurerm_mssql_firewall_rule" "firewall" {
  name             = "firewall-${random_integer.ri.result}"
  server_id        = azurerm_mssql_server.mssql.id
